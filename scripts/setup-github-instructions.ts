...
} from "../src/lib/services/github-instructions";
...
async function main() {
  ...
  console.log(
    "To use a different repository, modify the service configuration in:",
  );
  ...
  console.log(
    "💡 Pro Tip: Use comprehensive mode to fetch from multiple sources for complete coverage",
  );
}
...
async function testGitHubToken(token: string) {
  try {
    ...
    if (standardInstructions && standardInstructions.length > 0) {
      console.log(
        "✅ Standard service: Token is valid and can access repository",
      );
      console.log(
        `📖 Fetched ${standardInstructions.length} characters of instructions`,
      );
      ...
      console.log(
        "⚠️  Standard service: Token is valid but no instructions found",
      );
    ...
    const comprehensiveService = createCustomGPTPromptService(token);
    const comprehensiveInstructions =
      await comprehensiveService.fetchInstructions();

    if (comprehensiveInstructions && comprehensiveInstructions.length > 0) {
      console.log(
        "✅ Comprehensive service: Token is valid and can access repository",
      );
      console.log(
        `📖 Fetched ${comprehensiveInstructions.length} characters of comprehensive instructions`,
      );
      ...
      // Show difference
      const difference =
        comprehensiveInstructions.length - standardInstructions.length;
      if (difference > 0) {
        console.log(
      ...
      console.log(
        "⚠️  Comprehensive service: Token is valid but no comprehensive instructions found",
      );
    }
    ...
    try {
      ...
      console.log(
        `   Available instruction files: ${repoInfo.instructionFiles.length}`,
      );

      if (repoInfo.instructionFiles.length > 0) {
        console.log(
          `   Sample files: ${repoInfo.instructionFiles.slice(0, 5).join(", ")}`,
        );
      }
    } catch (error) {
      console.log(
        "⚠️  Repository metadata not accessible:",
